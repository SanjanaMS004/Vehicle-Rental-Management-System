...
def login_in():
	...
	#login_menu.bg = PhotoImage(file="image/newimg.png")
	#login_menu.bg_image = Label(login_menu, image=login_menu.bg).place(x=0, y=0, relwidth=2, relheight=2)

	k_font = tkinter.font.Font(family='Times new roman', size=16, weight=tkinter.font.BOLD)
	#orionLabel=Label(login_menu, text="VEHICAL RENTAL MANAGMENT SYSTEM",bg='plum1',font=("Castellar", "23","bold","italic","underline"),fg="black")
	#subLabel=Label(login_menu, text="BY SANJANA AND SANCHITHA",font=("Castellar", "16","bold","italic"))
	...
	loginbutton1=Button(login_menu,command=login_check,text=" Login ",bg='white',height='1',width='8', font=k_font,  bd = '5')
	registerbutton=Button(login_menu,command=register_in,text=" Register ",bg='black',fg='white',height='2',width='11',font=k_font, bd = '5',)
	#feedbackbutton=Button(login_menu,command=feedback_read,text=" Feedback ",bg='yellow',height='2',width='11',font=k_font, bd = '5')
	adminbutton=Button(login_menu,command=admin_in,text=" Admin Login ",bg='black', fg='white',height='2',width='11',font=k_font, bd = '5')
...
def add_check():
	...
	f22.close()
	key_sort('Bindex.txt')
	tkinter.messagebox.showinfo("Add","Vehicle added Successfully!")
	add_menu.destroy()
...
def del_vehicle():
	...
	borrow_label=Label(del_menu,text="Vehicle Id")
	borrow_label2=Label(del_menu,text="Vehicle")
	borrow_label3=Label(del_menu,text="Price")
	...
	with open("Bindex.txt",'r+') as file:
		...
		while line:
			...
			if words[0]==del_id:
				...
				index=file.tell()
				line=file.readline()
	tkinter.messagebox.showinfo("Delete","Vehicle Successfully removed")
	del_menu.destroy()
...
def Main_Menu():
	...
	base.wm_title("SAN Vehicle dealers")
	base.minsize(600,600)

	in_font = tkinter.font.Font(family='Garamond', size=15, weight=tkinter.font.BOLD)
	...
	orionLabel=Label(base, text="Street Smart Company",font=("Castellar", "35","roman","bold"),fg="darkslateblue")
	welcomeLabel=Label(base,text=("Welcome! "+id),font=("Garamond","30"))
	imageLibrary = PhotoImage(file="image/newimg.png")
	topFrame=Frame(base)
	...
	#Buttons
	borrow_but=Button(bottomFrame,bg="plum3",fg="black",text="Borrow Vehicle",font=in_font,height=5,width=17,command=borrow_in)
	return_but=Button(bottomFrame,bg="plum3",fg="black",text="Return Vehicle",font=in_font,height=5,width=17,command=return_in)
	search_but=Button(bottomFrame,bg="plum3",fg="black",text="Search for Vehicle",font=in_font,height=5,width=17,command=search_in)
...
def borrow_in():
	...
	borrow_label1=Label(borrow_menu,text="<<< Please enter the VEHICLE ID that you wish to borrow >>>",font=("Times", "20","bold","italic"),bg="light blue")
	borrow_label=Label(borrow_menu,text="Vehicle Id")
	borrow_label2=Label(borrow_menu,text="Vehicle")
	borrow_label3=Label(borrow_menu,text="Price")
...
def borrow_check():
	...
	for l in f:
		...
		if l[0] ==  id:
			count += 1
	if count >= 3:
		tkinter.messagebox.showinfo("Borrow", "Cannot have more than 3 Vehicle")
		borrow_menu.destroy()
		...
		if pos == -1:
			tkinter.messagebox.showinfo("Borrow","The Vehicle that you had entered is not in our database,sorry,please enter a different Vehicle")
			borrow_menu.lift()
		else:
			f2 = open('BData.txt', 'r+')
			f2.seek(pos)
			...
			if(w2[3] == 'Y'):
				...
				f2.write(l3)
				f2.close()
				tkinter.messagebox.showinfo("Borrow","The Vehicle you have selected has been successfully borrowed. Please return it by:" +'\n'+ str(enddate) )
				...
				f3.close()
				key_sort('Record.txt')
				Done2=tkinter.messagebox.askyesno("Borrow","Would you want to borrow another Vehicle?")
				if Done2==True:
					borrow_menu.destroy()
					borrow_in()
				else:
				...
				tkinter.messagebox.showinfo("Borrow","This Vehicle is currently unavailable, please select another Vehicle")
				borrow_menu.lift()
...
def return_in():
	...
	return_label=Label(return_menu,text="Vehicle Id")
	return_label2=Label(return_menu,text="Vehicle")
	return_label3=Label(return_menu,text="Price")
	...
	return_label1=Label(return_menu,text=" Please enter the Vehicle ID that you wish to return ",font=("Times", "12","bold","italic"),bg="light blue")
...
def return_check():
	...
	if(bbook in record_verification):
		...
		if pos != -1:
			...
			w1 = l1.split('|')
			if(w1[3] == 'N'):
				tkinter.messagebox.showinfo("Return","The Vehicle you have selected has been successfully returned on"+'\n'+str(date))
				f1.seek(pos)
				...
				for l2 in lines:
					...
					if l3[1] == bbook and l3[0] == id:
						...
						f3.write(l2)
				f3.close()
				Done3=tkinter.messagebox.askyesno("Return","Do you want to return another Vehicle?")
				if Done3==True:
					f1.close()
					return_menu.destroy()
					return_in()
				...
				tkinter.messagebox.showinfo("This Vehicle has been returned, please select another Vehicle")
			f1.close()
	else:
		tkinter.messagebox.showinfo("Return","The Vehicle that you had entered is invalid.Please reenter a different Vehicle")
		return_menu.lift()
...
def search_in():
	...
	search_label1=Label(search_menu,text="Search through our database to check if your desired Vehicle is available",font=("Times", "12","bold","italic"),bg="light blue")
...
def search_check():
	...
	if (pos == -1):
		tkinter.messagebox.showinfo("Search","Sorry,this Vehicle does not exist in our database")
	else:
		search_menu2=Tk()
